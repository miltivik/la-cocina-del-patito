...
interface PromptInputBoxProps {
  onSend?: (message: string, files?: File[], signal?: AbortSignal) => Promise<void> | void;
  onCancel?: () => void;
  isLoading?: boolean;
  placeholder?: string;
  className?: string;
  ...
  const handleSubmit = async () => {
    if (input.trim() || files.length > 0) {
      ...
      try {
        ...
        await onSend(formattedInput, currentFiles, controller.signal);
      } catch (error: any) {
  ...
  return (
    <>
      <PromptInput
        ...
        disabled={isLoading || internalIsLoading || isRecording}
            ...
            >
              <Button
                ...
                className={cn(
                  ...
                  isRecording
                    ? "bg-transparent hover:bg-red-500/10 text-red-600 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300"
                    : hasContent
                      ? "bg-blue-600 hover:bg-blue-700 text-white dark:bg-blue-500 dark:hover:bg-blue-600"
                      : "bg-gray-100 hover:bg-gray-200 text-gray-600 hover:text-gray-700 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-gray-300 dark:hover:text-gray-200"
                )}
                onClick={() => {
                  if (isLoading || internalIsLoading) handleCancel();
                  else if (isRecording) setIsRecording(false);
                  else if (hasContent) handleSubmit();
                  else setIsRecording(true);
                }}
                disabled={(isLoading || internalIsLoading) ? false : (!hasContent && !isRecording)}
              >
                {isLoading || internalIsLoading ? (
